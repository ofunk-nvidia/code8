...
import { ngcBaseUrl, openAiCodexDefaultModelId, openRouterDefaultModelId } from "@/shared/api"
...
export const AuthView: React.FC<AuthViewProps> = ({ controller, onComplete, onError, onNavigateToWelcome }) => {
	...
	const handleModelIdSubmit = useCallback(
		(value: string) => {
			...
			if (selectedProvider === "ngc") {
				setStep("saving")
				saveConfiguration(value, ngcBaseUrl)
			} else if (["openai", "openai-native"].includes(selectedProvider)) {
				setStep("baseurl")
			} else {
				setStep("saving")
