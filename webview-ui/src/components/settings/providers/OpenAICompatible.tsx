...
export const OpenAICompatibleProvider = ({
	...
	const effectiveBaseUrl = defaultBaseUrl || apiConfiguration?.openAiBaseUrl || ""
	const sortedAvailableModels = useMemo(() => [...availableModels].sort((a, b) => a.localeCompare(b)), [availableModels])
	...
	useEffect(() => {
		...
		if (apiConfiguration?.openAiBaseUrl !== defaultBaseUrl) {
			handleFieldChange("openAiBaseUrl", defaultBaseUrl)
		}

		if (apiConfiguration?.openAiApiKey) {
			debouncedRefreshOpenAiModels(defaultBaseUrl, apiConfiguration.openAiApiKey)
		}
	}, [
		defaultBaseUrl,
		...
		<div>
			<Tooltip>
				<TooltipTrigger>
					<div className="mb-2.5">
						...
						<DebouncedTextField
							disabled={lockBaseUrl || remoteConfigSettings?.openAiBaseUrl !== undefined}
							initialValue={effectiveBaseUrl}
							onChange={(value) => {
			...
			<ApiKeyField
				...
				onChange={(value) => {
					...
					debouncedRefreshOpenAiModels(effectiveBaseUrl, value)
				}}
			...
			<div style={{ display: "flex", gap: 8, alignItems: "end", marginBottom: 10 }}>
					...
					{sortedAvailableModels.length > 0 ? (
						<>
							...
							<VSCodeDropdown
								...
								{sortedAvailableModels.map((modelId) => (
									<VSCodeOption
				...
				<VSCodeButton
					...
					disabled={!effectiveBaseUrl || !apiConfiguration?.openAiApiKey}
					onClick={() => debouncedRefreshOpenAiModels(effectiveBaseUrl, apiConfiguration?.openAiApiKey)}>
					Refresh Models
				</VSCodeButton>
			</div>
