...
export function validateApiConfiguration(currentMode: Mode, apiConfiguration?: ApiConfiguration): string | undefined {
	if (apiConfiguration) {
		...
		switch (apiProvider) {
			...
			case "ngc":
				if (
					(apiProvider === "openai" && !apiConfiguration.openAiBaseUrl) ||
					(!apiConfiguration.openAiApiKey && !apiConfiguration.azureIdentity) ||
					!openAiModelId
				) {
					return apiProvider === "ngc"
						? "You must provide a valid NVIDIA API key and NIM model ID."
						: "You must provide a valid base URL, API key, and model ID."
				}
				break
			case "requesty":
